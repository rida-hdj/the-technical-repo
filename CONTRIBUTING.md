# دليل المساهمة: إضافة مقال جديد إلى المدونة

مرحبًا بك في المستودع التقني. هذا الدليل يشرح كيف تضيف مقالًا جديدًا إلى
المدونة بكل بساطة.

> قاعدة مهمة: المدونة تضيف الصفحات بشكل تلقائي لذا لا داعي لتعديل اي ملفات خارجية
> يجب كتابة كل المقالات بتنسيق ماركداون

---

## 1. شروط المساهمة

- أضف مقالك عبر **طلب دمج (Pull Request)** على GitHub.
- محتوى المقال **باللغة العربية الفصحى**.
- صورة المقال (إن وجدت) يجب أن تكون **بأبعاد 1280×720 بكسل** (عرضية) بصيغة JPG أو WebP.

---

## 2. مكان المقال

### مقال رئيسي

ضع ملف المقال في `src/content/posts/main/`:

```text
src/content/posts/main/my-article.md
```

### مقال قصير

المقالات القصيرة مرتبطة بمقال رئيسي. كل مجموعة قصيرة لها مجلد خاص بها
يحمل slug المقال الرئيسي:

```text
src/content/posts/small/{slug-of-main-article}/
```

مثلاً لمقال رئيسي بعنوان "كيف يعمل الحاسوب" (`how-a-computer-work`):

```text
src/content/posts/small/how-a-computer-work/
├── 1.md
├── 2.md
└── 3.md
```

---

## 3. بنية ملف المقال

### المقال الرئيسي

```yaml
---
title: "كيف يعمل الحاسوب"
author: "اسمك الأول"
authorGithub: "your-github-username"
description: "مقدمة الى كيف يعمل الحاسوب"
type: "main"
small: "how-a-computer-work"    # slug مجلد المقالات القصيرة (اختياري)
pubDate: 2026-08-20
tags:
  - الحاسوب
  - العتاد
image: "/images/computer.jpg"
---
```

| الحقل | الوصف |
|---|---|
| `title` | عنوان المقال |
| `author` | اسم الكاتب |
| `authorGithub` | اسم المستخدم على GitHub |
| `description` | وصف موجز للمقال |
| `type` | `main` |
| `small` | slug مجلد المقالات القصيرة المرتبطة (اختياري) |
| `pubDate` | تاريخ النشر |
| `tags` | وسوم المقال |
| `image` | مسار الصورة (اختياري) |

### المقال القصير

```yaml
---
title: "ما هو المعالج؟"
author: "اسمك الأول"
authorGithub: "your-github-username"
description: "شرح مفصل لما هو المعالج ودوره في الحاسوب"
type: "small"
order: 1
pubDate: 2026-08-20
tags:
  - الحاسوب
  - العتاد
image: ""
---
```

| الحقل | الوصف |
|---|---|
| `title` | عنوان المقال |
| `author` | اسم الكاتب |
| `authorGithub` | اسم المستخدم على GitHub |
| `description` | وصف موجز للمقال |
| `type` | `small` |
| `order` | رقم الترتيب داخل المجموعة (يفضل لو كان يبدأ من 1) |
| `pubDate` | تاريخ النشر |
| `tags` | وسوم المقال |
| `image` | مسار الصورة (اختياري) |


---

## 4. إضافة صورة للمقال

الصورة اختيارية لكنها تضيف لمسة بصرية للمقال

1. جهّز صورة **عرضية** بأبعاد **1280×720 بكسل** بصيغة (JPG أو WebP)
2. ضع الملف في مجلد الصور العام:

   ```text
   public/images/my-article.png
   ```

3. أشر إلى الصورة في المقدمة (Frontmatter) بمسارها المبدئي بـ `/`:

   ```yaml
   image: "/images/my-article.png"
   ```

إذا لم ترد صورة، احذف سطر `image` تمامًا أو اتركه فارغًا `""`.

---

## 5. مثال: إضافة مجموعة مقالات قصيرة

لنفترض أن لديك مقالًا رئيسيًا بعنوان "مقدمة إلى لينيكس" (`introduction-to-linux`):

### الخطوة 1: أضف المقال الرئيسي

```text
src/content/posts/main/introduction-to-linux.md
```

```yaml
---
title: "مقدمة إلى لينيكس"
author: "اسمك الأول"
authorGithub: "your-github-username"
description: "مقدمة شاملة لنظام لينيكس"
type: "main"
small: "introduction-to-linux"
pubDate: 2026-08-20
tags:
  - لينيكس
  - مفتوح المصدر
---
```

### الخطوة 2: أنشئ مجلد المقالات القصيرة

```text
src/content/posts/small/introduction-to-linux/
```

### الخطوة 3: أضف المقالات القصيرة مع ترتيب

```text
src/content/posts/small/introduction-to-linux/1.md
```

```yaml
---
title: "ما هو لينيكس؟"
author: "اسمك الأول"
authorGithub: "your-github-username"
description: "تعريف نظام لينيكس وتاريخه"
type: "small"
order: 1
pubDate: 2026-08-20
tags:
  - لينيكس
---
```

```text
src/content/posts/small/introduction-to-linux/2.md
```

```yaml
---
title: "توزيعات لينيكس"
author: "اسمك الأول"
authorGithub: "your-github-username"
description: "شرح أشهر توزيعات لينيكس"
type: "small"
order: 2
pubDate: 2026-08-20
tags:
  - لينيكس
---
```

### التنقل بين المقالات القصيرة

- المقال الرئيسي يقود إلى أول مقال قصير.
- المقال القصير رقم 1 يقود إلى رقم 2.
- المقال القصير الأخير يقود إلى رقم السابق فقط.

---

## 6. قواعد الترتيب والتحقق

- `order` يجب أن يكون **رقمًا موجبًا** (0 مسموح).
- لا يجب تكرار أرقام الترتيب داخل مجموعة واحدة.
- إذا نشرت مقالًا قصيرًا بدون `order`، سيظهر تحذير في سجلات البناء.
- إذا أشرت إلى مجلد قصير غير موجود، سيظهر تحذير في سجلات البناء.

---

## 7. الاختبار محليًا قبل الإرسال

```sh
npm install       # تثبيت الاعتماديات (أول مرة فقط)
npm run dev       # تشغيل الخادم المحلي على http://localhost:4321
```

تأكد من أن:

- صفحة المقال تعمل بشكل صحيح.
- المقالات القصيرة مرتبطة بالمقال الرئيسي.
- التنقل السابق/التالي يعمل في المقالات القصيرة.
- الصورة تظهر بشكل صحيح.

قبل إنشاء طلب الدمج، شغّل فحص المشروع:

```sh
npm run build
npm run astro check
```

يجب أن يكتمل البناء بدون أخطاء.

---

## 8. خطوات إرسال المقال عبر GitHub

1. انسخ المستودع (Fork) ثم استنسخه محليًا.
2. أنشئ فرعًا جديدًا، مثل: `add-post-introduction-to-linux`.
3. أضف ملف المقال في المسار الصحيح.
4. ارفع التغييرات إلى GitHub.
5. افتح **Pull Request** إلى فرع `main` مع وصف موجز للمقال.
6. بعد مراجعة المقال ودمجه، ستظهر صفحته تلقائيًا بعد البناء — دون أي تعديلات
   أخرى.
